...
class Site_4_publication:
    def __init__(self, clf_file=None):
        ...
        self.text_stammer = nltk.stem.SnowballStemmer('russian')           
        self.text_load()
        if not clf_file:
    ...
    def text_open(self, text_file, Geek_flag):
        ...
        for i in text['text']:
            stem = " ".join([self.text_stammer.stem(word.lower()) for word in i.split()])
            self.text.append(stem)
        if Geek_flag == 1:
    ...
    def classification_train(self):
        X_train, X_test, y_train, y_test = sklearn.cross_validation.train_test_split(self.X_dat,\
                                      self.y, test_size=0.2, random_state=246)    
        clf = sklearn.svm.LinearSVC(C = 1.0, penalty='l2')
        self.my_svm = clf.fit(X_train, y_train)        
